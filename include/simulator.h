...
#define _SIMULATOR_UNROLL_MAX_BUFF_CNT UNROLL_LOOP(kRoomMaxBuffSlots)
...
namespace albc
{
	...
	{
	public:
		static double DoCalc(const RoomModel *room, double max_allowed_duration)
			__attribute__((const))
			__attribute__((always_inline))
		{
			...
			_SIMULATOR_UNROLL_MAX_BUFF_CNT
			for (UInt32 i = 0; i < room->n_buff; ++i)
			{
				auto &applier = room->buffs[i]->applier;
				if (applier.NeedUpdateScope(scope))
					applier.UpdateScope(scope);
			}
			...
			for (UInt32 i = 0; i < room->n_buff; ++i)
			{
				...
				estimated_duration = std::min(estimated_duration,
											  cost_mul > 0 ? room->buffs[i]->duration / cost_mul : estimated_duration);
			}
			...
			for (UInt32 i = 0; i < room->n_buff; ++i)
			{
				...
				{
					const double acc = buff_mod.eff_inc_per_hour * 2.777777777777778e-4; // devide by 3600, unit: 1/s
					base_acc += acc;
					if (const double acc_finish_ts = abs(buff_mod.max_extra_eff_delta / base_acc); acc_finish_ts <
																								   estimated_duration)
			...
			double final_eff_delta = room->room_attributes.base_prod_eff; // final effective delta, unit: 1
			...
			for (UInt32 i = 0; i < room->n_buff; ++i)
			{
				...
				case RoomFinalAttributeModifierType::OVERRIDE_AND_CANCEL_ALL: // override
					if (override == nullptr
                        ||!RoomFinalAttributeModifier::validate(*override)
			...
			if (override != nullptr && override->IsValid())
			{
				return estimated_duration * ((room->room_attributes.base_prod_eff + override->eff_delta) * override->eff_scale * indirect_eff_mul + indirect_eff_delta);
			}

			// insert parameter at t = 0
			eff_piecewise.Insert(0., base_eff_delta, base_acc, final_eff_mul * indirect_eff_mul, final_eff_delta + indirect_eff_delta); 
            ...
            for (UInt32 i = 0; i < room->n_buff; ++i)
            {
                ...
                {
                    const double reach_top_ts = final_mod.max_extra_eff_delta <= perv_base * final_mod.eff_scale
                                                    ? perv_ts
                                                    : fp_round_n(
                                                          (final_mod.max_extra_eff_delta / final_mod.eff_scale -
                                                           perv_base) /
                                                          perv_acc);
                    // handle a multiplier of other buffs reaching its maximum value because the effective increment over time_t
                    if (reach_top_ts >= 0 && reach_top_ts <= ts)
                    {
                        // pending_piecewise.push(std::make_pair(reach_top_ts, PiecewiseDef(0., 0., 1 / final_mod.eff_scale, final_mod.max_extra_eff_delta)));
                        eff_piecewise.Insert(reach_top_ts, 0., 0., 1. / final_mod.eff_scale,
			...
			return Integrate(eff_piecewise, 0., estimated_duration); // integrate the piecewise function
		}
	...
	protected:
		...
		__attribute__((const))
		__attribute__((always_inline))
		Integrate(const PiecewiseMap<kFuncPiecewiseMaxSegmentCount> &map,
...

		{
			...
			double extra = origin_def.extra_delta;

			if (map.n == 1)
			{
				return (mul * base + mul * acc * (lower_bound + upper_bound) * .5 + extra) 
			...
			{ // NOTE: the first segment is not included, so skip it
				...
				//(f(calc_lb) + f(calc_ub)) * (calc_ub - calc_lb) / 2
				//((mul * (base + acc * calc_lb) + extra) + (mul * (base + acc * calc_ub) + extra)) * (calc_ub - calc_lb) / 2
				//(2mul * (base) + mul * acc * (calc_lb + calc_ub) + 2extra) * (calc_ub - calc_lb) / 2
				...
				// aggregates current segment and the previous one
			}
