...
namespace albc
{
	...
	public:
        ...
        OperatorModel(const PlayerCharacter& player_char, const bm::PlayerBuildingChar& building_char,
            ...
            room_type_mask(bm::RoomType::NONE),
		...
		void Empower(const PlayerTroopLookup& lookup, const PlayerCharacter& player_char, const bm::BuildingData& building_data,
            const bool error_on_buff_not_found = false, const bool igonre_unlock_cond = false)
			...
			{ // BuffChar : 角色可以同时拥有的不同buff槽位
				...
				{ // BuffData : 每个Buff槽位中对应角色当前等级数据的Buff
					...
					if (!BuffMap::instance()->count(buff_data.buff_id))
					{
                        ...
                        {
                            LOG_E << "Unable to find buff with given ID! Is buff data outdated? : " << buff_data.buff_id << endl;
                        }
						continue;
					}

					if (igonre_unlock_cond || buff_data.cond.Check(evolve_phase, level))
					{
    ...
    private:
		void ResolvePatches() 
		{
            ...
            buffs.erase(std::remove_if(buffs.begin(), buffs.end(), [patch_target](const RoomBuff* buff) -> bool {
				bool remove = patch_target.count(buff->buff_id);
				if (remove)
                {
					LOG_D << "Removing buff " << buff->buff_id << " from operator " << buff->owner_char_id << endl;
				}
