...
namespace albc{
    constexpr UInt32 kInvalidSlot = static_cast<unsigned>(0xFFFF);

    // check if the slot is valid, indicating a valid, or an "occupied" slot
    constexpr auto validate_room_slot(const UInt32 slot) -> bool
    {
    ...
    struct __attribute__((aligned(64))) __attribute__((packed)) RoomAttributeFields
    {
        ...
        double base_prop_cap = 10;//基础产品容量
    ...
    struct __attribute__((aligned(64))) __attribute__((packed)) RoomAttributeModifier
    {
        ...
        static constexpr void init(RoomAttributeModifier &modifier, RoomBuff *buff,
                                   const RoomBuffType buff_type, const double eff_delta = 0,
                                   const int cap_delta = 0, const double eff_delta_inc_hour = 0,
        ...
        [[maybe_unused]]
        static constexpr void mark_invalid(RoomAttributeModifier &modifier)
        {
    ...
    struct __attribute__((aligned(16))) __attribute__((packed)) RoomFinalAttributeModifier : RoomAttributeModifier
    {
        ...
        static constexpr void init(RoomFinalAttributeModifier &modifier, RoomBuff *buff, const RoomBuffType buff_type,
                                   const double eff_delta,
                                   const int cap_delta, const double eff_delta_inc_hour,
                                   const double max_extra_eff_delta,
                                   const RoomFinalAttributeModifierType final_mod_type, const double eff_scale = 1)
        {
    ...
    struct __attribute__((aligned(32))) CharacterCostModifier
    {
        ...
        static constexpr void init(CharacterCostModifier &modifier, RoomBuff *buff,
                                   const CharCostModifierType type, const double value)
        {
            modifier.owner = buff;
            modifier.type = type;
    ...
    public:
        ...
        {
            // return Store(buffs, kRoomMaxBuffs, next_buff_slot, [](const auto& ptr) -> bool{ return ptr != nullptr; }, buff);
            buffs[n_buff] = buff;
            n_buff++;
        }
    ...
    public:
        ...
        virtual auto validate(const RoomModel * /*room*/) -> bool { return true; }
    };
