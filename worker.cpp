...
{
    bool debug = logLevel <= LogLevel::DEBUG;
    ...
    if (!debug)
    {
        LOG_W << unsupported_buff_cnt
              << R"( unsupported buff found in building data buff definitions. Add "--debug"("-d") param to check all.)"
              << std::endl;
    }
    ...
    try
    {
        ...
        LOG_I << "Added " << player_data_model->building.player_building_room.manufacture.size() << " factories."
              << std::endl;
        LOG_I << "Added " << player_data_model->building.player_building_room.trading.size() << " trading posts."
              << std::endl;
    ...
    for (const auto &[inst_id, player_char] : player_data_model->troop.chars)
    {
        ...
        {
            LOG_E
                << "Unable to find building data definition for character with given ID! Is building data outdated? : "
                << player_char->char_id << endl;
            continue;
        }

        const auto &op =
            new OperatorModel(*player_char, *player_data_model->building.chars.at(inst_id), *building_data);
